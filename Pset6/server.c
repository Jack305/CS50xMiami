...
char* indexes(const char* path)
{
    // allocates memory for the copy of the path 
    char* pathCopy = malloc(strlen(path) + strlen("index.html")+1);
    strcpy(pathCopy, path);
    
    // checks if there is '/' at the end of the path
    if (pathCopy[strlen(path)-1] != '/')
    {
        // adds '/' if it doesn't exist
        pathCopy = strcat(pathCopy, "/");    
    }
    
    // creates path to index.php
    char* fullPath = malloc(strlen(pathCopy));
    strcpy(fullPath, pathCopy);
    strcat(fullPath, "index.php");

    // checks if file index.php exist and returns the path if true
    if (access(fullPath, F_OK) != -1)
        return fullPath;
    
    // creates path to index.html
    char* fullPath2 = malloc(strlen(pathCopy));
    strcpy(fullPath2, pathCopy);
    strcat(fullPath2, "index.html");
    
    // checks if file index.html exist and returns the path if true
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // contols the status of file reading
    bool status = false;
    
    // counts th elength
    *length = 0;
    
    // buffer for reading bytes from the file
    char* pipe = malloc(1);
    *content = malloc(1);

    // reads byte after byte and puts the information into *CONTENT
    while(fread(pipe, 1, 1, file) == 1)
    {
        // copys information from pipe to a next allocated space
        memcpy(*content + *length, pipe, 1);
        *length += 1;
        // reallocates the memory - adds 1 more byte
        *content = realloc(*content, *length + 1); 
    }  

    // checks if content isn't empthy
    if (*content != NULL)
        status = true;
...
const char* lookup(const char* path)
{
    // takes the extention of the path
    char* buffer1 = strchr(path, '.');
    // checks if extention of the path matches to any of the correct extentions
    if (strcasecmp(buffer1, ".css") == 0) return "text/css";
    if (strcasecmp(buffer1, ".html") == 0) return "text/html";
    if (strcasecmp(buffer1, ".gif") == 0) return "image/gif";
    if (strcasecmp(buffer1, ".ico") == 0) return "image/x-icon";
    if (strcasecmp(buffer1, ".jpg") == 0) return "image/jpeg";
    if (strcasecmp(buffer1, ".js") == 0) return "text/javascript";
    if (strcasecmp(buffer1, ".php") == 0) return "text/x-php";
    if (strcasecmp(buffer1, ".png") == 0) return "image/png";
    return NULL;
}
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // checks if request-target contains "
    if (strchr(requestTarget, '"'))
    {
        error(400);
        return false;
    } 
    ...
    // checks if 1st char is not '/' or there are 2 spaces between strings in line
    if (requestTarget[0] != '/' || 
       (requestTarget+strlen(requestTarget)+1)[0] != 'H' || 
       (requestTarget-2)[0] != 'T')
    {
        error(501);
        return false;
    ...
    // if line satisfys all requirements
    else
    {
        // checks if there is '?' - query
        if (strchr(requestTarget, '?'))
        {
            strcpy(abs_path, strtok(requestTarget, "?"));

            // takes the string after '?'
            char* buf = strtok(NULL, " ");
            
            // checks if string after '?' isn't = NULL
            if (!buf)
            {
                // if query == NULL
                strcpy(query,""); 
...
void transfer(const char* path, const char* type)
{
    ...
    respond(301, headers, content, length);
